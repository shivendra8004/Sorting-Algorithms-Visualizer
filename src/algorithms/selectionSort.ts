...
export function generateSelectionSortArrayAnimation(isSorting: boolean, array: number[], runAnimation: (animations: AnimationArrayType) => void) {
    if (isSorting) return;
    if (array.length <= 1) return;

    const animations: AnimationArrayType = [];
    const auxialiaryArray = array.slice();
    runSelectionSort(auxialiaryArray, animations);
